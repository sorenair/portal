...
const imageUrlNative = 'https://sorendev.com/api/dev1/img'; // Replace with your API image URL
const savePathNative = path.join(__dirname, 'downloads', 'media.jpg'); // Save in a 'downloads' subfolder

function downloadImageNative(imageUrl, filepath) {
...
async function syncConfig() {
  ...
  try {
      const response = await fetch('https://sorendev.com/api/dev1/params', {
          method: 'GET'
      });
      ...
      if (config[0].state == 'true') {
          ...
          try {
              const response = await fetch('https://sorendev.com/api/dev1/update', {
                  method: 'POST',
                  body: 'false',
                  headers: {
    ...
    exec('pidof led-image-viewer', (error, stdout, stderr) => {
      if (error) {
        ...
        exec('pidof text-scroller', (error, stdout, stderr) => {
          if (error) {
            // if led-viewer NOT active AND text-scroller NOT active
            exec(`sudo /home/soren/matrix/rpi-rgb-led-matrix/utils/text-scroller -y5 --led-rows=32 --led-cols=64 -f /home/soren/matrix/rpi-rgb-led-matrix/fonts/9x18.bdf ${message}`);
            return;
          }
          else {
            // if led-viewer NOT active AND text-scroller IS active -> kill text-scroller
...
function updatePower () {
  ...
  if (power == 'off') {
    ...
    exec(`sudo /home/soren/matrix/rpi-rgb-led-matrix/utils/led-image-viewer -C --led-rows=32 --led-cols=64 --led-swap-green-blue --led-brightness=${brightness.toString()} /home/soren/matrix/portal/images/idle.gif`);
    return;
  }
}

//const intervalId = setInterval(downloadImageNative, 5000, imageUrlNative, savePathNative);
