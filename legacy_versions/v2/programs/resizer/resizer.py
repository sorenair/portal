...
def main():
    ...
    with open(f'{cwd}/programs/resizer/test.txt', "w") as file:
        file.write("success")

    # Check if uploaded file is an image or GIF
    for filename in os.listdir(f'{cwd}/uploads'):
        # Handle images
        if (filename[-3:].lower() == 'jpg') or (filename[-3:].lower() == 'png') or (filename[-4:].lower() == 'jpeg') or (filename[-4:].lower() == 'heif'):
            img = cv.imread(f'{cwd}/uploads/{filename}', cv.IMREAD_COLOR)
            resized_img = resize(img)
            cv.imwrite(f'{cwd}/uploads/{filename}',resized_img)
        ...
        # Handle GIFs
            ...
            with image.open(f'{cwd}/uploads/{filename}') as img:
                frame = 0
                while True:
            ...
            frames[0].save(f'{cwd}/uploads/image.gif', save_all=True, append_images=frames[1:], optimize=False, duration=100, loop=0)
