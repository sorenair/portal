let preview = document.getElementById('device_image');
...
fileInput.addEventListener('change', async () => {
    ...
    try {
        const response = await fetch('/upload/oriel', {
        method: 'POST',
        body: formData,
        });
...
document.getElementById('message-form').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        const response = await fetch('/upload_text/oriel', {
            method: 'POST',
            body: textFormData,
            headers: {
...
let powerSlider = document.getElementById("powerSlider").addEventListener('click', async function(event) {
    ...
    try {
        // Use Fetch to send the form data asynchronously
        const response = await fetch('/power/oriel', {
            method: 'POST',
            body: powerStatus,
            headers: {
            'Content-Type': 'text/plain'
...
brightness.addEventListener('mouseup', async function(event) {
    try {
        // Use Fetch to send the form data asynchronously
        const response = await fetch('/brightness/oriel', {
            method: 'POST',
...
window.addEventListener("load", async function(event) {
    try {
        const response = await fetch('/api/oriel/params', {
            method: 'GET'
        });
        ...
        brightness.value = config[1].state;
        brightness_value.innerHTML = config[1].state;

        if (config[2].state == 'on') {
        ...
        console.log(config[2].state);

        filename = config[5].state
        preview.src = `../uploads/oriel/${filename}?t=${Date.now()}`;
    } catch (error) {
        alert('An error occurred during submission.');
    }    
});